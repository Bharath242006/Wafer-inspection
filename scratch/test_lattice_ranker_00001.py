...
def test_lattice_ranker_00001():
    ...
    # 3. Calculate Trained Global/Lattice scores
    t_lat_start = time.perf_counter()
    lat_scores = compute_global_lattice_scores(ref_raw, search_raw, cands_500, checkpoint_path=checkpoint_path)
    lat_time_sec = time.perf_counter() - t_lat_start

    # Attach Lattice scores to candidate records
    for i, c in enumerate(cands_500):
        c['lattice_score'] = lat_scores[i]
        # Combined score: 0.60 * lattice_score + 0.40 * final_score
        c['combined_score'] = 0.60 * c['lattice_score'] + 0.40 * c['final_score']
    ...
    print("=" * 135)
    print("           TRAINED GLOBAL/LATTICE-AWARE CANDIDATE RANKER DIAGNOSTIC TEST FOR 00001.PNG")
    print("=" * 135)
    ...
    print(f"Lattice Scoring Time:        {lat_time_sec*1000.0:.2f} ms ({lat_time_sec:.4f} s)")
    print(f"Total Computation Runtime:   {elapsed_time_sec*1000.0:.2f} ms ({elapsed_time_sec:.4f} s)")
    print("-" * 135)

    print(f"{'Rank':<5} | {'Candidate Center (x, y)':<24} | {'Dist to GT (px)':<18} | {'Handcrafted Score':<18} | {'Lattice Score':<16} | {'Combined Score':<16}")
    print("-" * 135)

    for r, c in enumerate(cands_combined[:10], start=1):
        d_gt = math.hypot(c['cx'] - gt_x, c['cy'] - gt_y)
        coord_str = f"({c['cx']:.2f}, {c['cy']:.2f})"
        print(f"#{r:<4} | {coord_str:<24} | {d_gt:<18.2f} | {c['final_score']:<18.4f} | {c['lattice_score']:<16.4f} | {c['combined_score']:<16.4f}")
    ...
    print(f"LATTICE-COMBINED TOP:        ({top_comb['cx']:.2f}, {top_comb['cy']:.2f}) | Handcrafted: {top_comb['final_score']:.4f} | Lattice: {top_comb['lattice_score']:.4f} | Combined: {top_comb['combined_score']:.4f}")
    print(f"GROUND TRUTH COORDINATE:     ({gt_x:.2f}, {gt_y:.2f})")
    print(f"PIXEL ERROR TO GT:           {pred_err:.2f} px")
    print(f"LATTICE RANKER CHANGED WINNER? {'YES' if changed else 'NO'}")
    print(f"STATUS:                      SUCCESS")
    print("=" * 135)

if __name__ == "__main__":
    test_lattice_ranker_00001()
